...
void listarVinhos() {
    ...
    // Loop principal do menu de listagem
    do {
        limparTela();
        
        // 1. Mostra a lista simples
        ...
        if (id_digitado == 0) {
            ...
            // Mostra os detalhes do ID solicitado
            listarDetalhesVinho(con, (int)id_digitado);
            pausarTela(); // Pausa para o usuário ler os detalhes
        }
        
    } while (id_digitado != 0); // O loop continua até o usuário digitar 0
...
void atualizarVinho() {
    ...
    Vinho v;
    MYSQL *con = conectar_db(); // <-- MUDANÇA: Abre a conexão no início
    MYSQL_STMT *stmt;
    MYSQL_BIND bind[7];
    ...
    // --- MUDANÇA AQUI ---
    // Chamando a função de listagem SIMPLES, passando a conexão
    listarVinhosSimples(con); 
    // --- FIM DA MUDANÇA ---
    ...
    if (end_ptr == input_buffer || *end_ptr != '\0') {
        printf("\nErro: Entrada invalida. Apenas numeros sao permitidos.\n");
        mysql_close(con); // Fecha a conexão antes de sair
        return; 
    }
    ...
    // --- 3. SE O ID FOR VÁLIDO, CONTINUA ---
    // (Não precisa mais conectar, já conectamos)
...
void atualizarUva() {
    ...
    // --- 2. COLETAR E VALIDAR O ID ---
    printf("\nID da Uva a atualizar (ou 0 para cancelar): ");
    scanf("%100[^\n]", input_buffer);
    while(getchar() != '\n'); 
...
/*
 * (NOVA FUNÇÃO AUXILIAR 1)
 * Mostra a lista SIMPLES de vinhos (ID, Nome, Safra, Preço, Tipo)
 * Esta função RECEBE uma conexão.
 */
void listarVinhosSimples(MYSQL *con) {
